...
function GenKeyAndCSR(){
    ...
    openssl rsa -passin pass:${PASSWORD} -in $1/private_enc.pem -out $1/private_denc.pem
    openssl req -new -key $1/private_denc.pem -out $1/_request.csr -subj "/C=${C}/ST=${ST}/L=${L}/O=${O}/CN=$2"
}
...
function IssuringCert(){
    ...
    case "$2" in
        1) #RootCA
            openssl ca -in $1/_request.csr -out $1/$1.crt -days ${EXPIRE_DAYS} -config RootCA/conf.cnf -extfile $1/_san.conf 
            ;;
        2) #IntermediateCA
            openssl ca -in $1/_request.csr -out $1/$1.crt -days ${EXPIRE_DAYS} -config IntermediateCA/conf.cnf -extfile $1/_san.conf 
            ;;
    esac

    #Generate PKCS12 File
    openssl pkcs12 -export -in $1/$1.crt -inkey $1/private_denc.pem -out $1/$1_pkcs12.pfx -passout pass:${PASSWORD} 
    rm -f $1/_request.csr 
    rm -f $1/_san.conf
}
...
crl             = $1/crl.pem         
private_key     = $1/private_denc.pem
...
if [ ! -d RootCA ]; then
    ...
    openssl x509 -days ${EXPIRE_DAYS} -in RootCA/_request.csr -req -signkey RootCA/private_denc.pem -out RootCA/RootCA.crt
...
crl             = RootCA/crl.pem         
private_key     = RootCA/private_denc.pem
default_md      = sha256                
policy          = policy_any
[ policy_any ]
countryName             = optional
...
" > IntermediateCA/conf_for_IntermediateCA.cnf
    ...
    openssl ca -in IntermediateCA/_request.csr -out IntermediateCA/IntermediateCA.crt -days ${EXPIRE_DAYS} -config IntermediateCA/conf_for_IntermediateCA.cnf 
...
while :
    ...
    clear
    echo "************************************************************"
    echo "Issuring Server Certificate Script - last updated: 2020/9/7"
    echo "************************************************************"
    echo ""
    ...
    4)
        ...
        cat IntermediateCA/private_denc.pem
    ...
    5)
        ...
        echo "Input domain"
        read checkDomain
        openssl x509 -noout -text -in ${checkDomain}/${checkDomain}.csr
        read
        ;;
    9)
